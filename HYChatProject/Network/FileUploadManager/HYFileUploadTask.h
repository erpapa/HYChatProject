...
/* 上传任务的状态 */
typedef NS_ENUM(NSUInteger, HYFileUploadState) {
...
/* 文件上传任务 */
@interface HYFileUploadTask : NSObject<NSCoding>

/* 当前状态 */
@property (nonatomic,assign)HYFileUploadState uploadState;
...
/* 需要上传文件的对象数组，包含HYUploadFileModel */
 ...
 */
+ (HYFileUploadTask *)taskWithFileData:(NSData*)fileData withFileName:(NSString*)fileName withFormName:(NSString*)formName taskObserver:(NSObject*)observer getTaskUniqueIdentifier:(NSString**)taskIdentifier;

/*
 * 指定特殊这个任务特殊的观察者情况下: 文件组上传任务
 */
+ (HYFileUploadTask *)taskWithMutilFile:(NSArray*)fileModelArray taskObserver:(NSObject*)observer getTaskUniqueIdentifier:(NSString**)taskIdentifier;
